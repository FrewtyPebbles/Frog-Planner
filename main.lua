...
function printCSV()
	for i = 1, table.getn(plannerData) do
		io.write("########################################~~~~~~~~~~~~~~~"..plannerData[i][1].."~~~~~~~~~~~~~~~VVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVVV\n")
		for j = 2, table.getn(plannerData[i]),2 do
			io.write(" -=- "..plannerData[i][j].." > "..plannerData[i][j+1].."\n\n")
		end
		io.write("________________________________________")
	end
end
...
--REMOVE TASK PROCESS
function removeTaskProcess(date, time, task)
	for i = 1, table.getn(plannerData) do
		if date == plannerData[i][1] then
			--remove from table
			for j = 2, table.getn(plannerData[i]),2 do

				if plannerData[i][j] == time then
					table.remove(plannerData[i], j)
					table.remove(plannerData[i], j)
					break
				end
...
--REMOVE TASKDate PROCESS
function removeTaskDateProcess(date, time, task)
	for i = 1, table.getn(plannerData) do
		if date == plannerData[i][1] then
			--remove from table
			table.remove(plannerData, i)
			break
		end
	end
	for i = 1, table.getn(plannerData) do
...
function appendData(data, date, time, task, process)
	...
	elseif process == "remove task" then

		removeTaskProcess(date,time,task)

	elseif process == "remove taskDate" then

		removeTaskDateProcess(date,time,task)
